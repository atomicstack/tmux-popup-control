...
// extractReloadCmd re-captures+extracts the current category asynchronously,
// updating the current level in place (no new stack level, filter query
// preserved). Bumps m.extractSeq and stamps the request with it so
// handleExtractReloadMsg can ignore out-of-order replies from a rapid mode
// change (see internal/ui/preview.go for the same seq pattern).
func (m *Model) extractReloadCmd() tea.Cmd {
	m.extractSeq++
	seq := m.extractSeq
	ctx := m.menuContext() // carries the current ExtractCategory
